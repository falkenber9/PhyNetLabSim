...
Define_Module(EnergyTracker);

simsignal_t EnergyTracker::totalEnergySignal = registerSignal("totalEnergy");
simsignal_t EnergyTracker::powerConsumtionSignal = registerSignal("powerConsumtion");

void EnergyTracker::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        EV << "[BATTERY]: " << "capacity = " << capmAh << "mA-h (nominal = " << nominalCapmAh << ") at " << voltage << "V" << endl;
        EV << "[BATTERY]: " << "publishDelta = " << publishDelta * 100 << "%, publishTime = " << publishTime << "s, resolution = " << resolution << "sec" << endl;
...
int EnergyTracker::registerDevice(cObject *id, int numAccts)
{
    ...
    EV << "[BATTERY]: " << "initialized device "  << deviceEntryVector.size() << " with " << numAccts << " accounts" << endl;
    deviceEntryVector.push_back(device);
    return deviceEntryVector.size()-1;
}

void EnergyTracker::registerWirelessDevice(int id, double mUsageRadioIdle, double mUsageRadioRecv, double mUsageRadioSend, double mUsageRadioSleep)
{
    Enter_Method_Silent();
    if (deviceEntryMap.find(id)!=deviceEntryMap.end())
    {
        EV << "[BATTERY]: " << "This device is register \n";
        return;
    }
...
void EnergyTracker::handleMessage(cMessage *msg)
{
...
double EnergyTracker::getConsumedEnergy()
{
    return (capacity - residualCapacity);
}

double EnergyTracker::getVoltage()
{
    return voltage;
}

void EnergyTracker::receiveChangeNotification(int aCategory, const cObject* aDetails)
{
    Enter_Method_Silent();
    ...
    if (aCategory == NF_RADIOSTATE_CHANGED)
    {
        ...
        emit(powerConsumtionSignal, current*voltage); // mW

        EV << "[BATTERY]: " << simTime() << " wireless device " << rs->getRadioId() << " draw current " << current <<
...
void EnergyTracker::draw(int deviceID, DrawAmount& amount, int activity)
{
    if (amount.getType() == DrawAmount::CURRENT)
    {
        ...
        EV << "[BATTERY]: " << simTime() << " device " << deviceID <<
    ...
    else if (amount.getType() == DrawAmount::ENERGY)
    {
        ...
        EV << "[BATTERY]: " << simTime() << " device " << deviceID <<  " deduct " << energy << " mW-s, activity = " << activity << endl;
...
void EnergyTracker::deductAndCheck()
{
    ...
    emit(totalEnergySignal, ((capacity - residualCapacity)/60/60)); //mWh

    //EV << "[BATTERY]: " << "residual capacity = " << residualCapacity << "\n";
    EV << "[BATTERY]: " << "consumed energy   = " << (capacity - residualCapacity) << "mWs\n";
    //EV << "[BATTERY]: " << "consumed energy   = " << (capacity - residualCapacity)/60/60 << "mWh\n";
    //EV << "[BATTERY]: " << "consumed energy   = " << (capacity - residualCapacity)/60/60/voltage << "mAh\n";
    ...
    totalConsumedEnergy = (capacity - residualCapacity); // mWs
}
