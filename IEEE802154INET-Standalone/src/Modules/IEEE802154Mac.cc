...
void IEEE802154Mac::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        //XXX added by me
        mpib.setMacMaxBE(par("maxBE"));
        mpib.setMacMinBE(par("minBE"));
        mpib.setMacMaxCSMABackoffs(par("maxCSMABackoffs"));
        mpib.setMacMaxFrameRetries(par("maxFrameRetries"));
...
void IEEE802154Mac::handleUpperMsg(cMessage *msg)
{
    ...
    if (dynamic_cast<cPacket *>(msg) != NULL)
    {
        ...
        if (mcpsData->getByteLength() > aMaxMACFrameSize)
        {
            ...
            sendMCPSDataConf(mac_FRAME_TOO_LONG, msg->getId());    // TODO do not send the OMNeT message ID as it will overflow the unsigned char msdu handle quickly
            delete msg;
            return;
        }
    }
...
//                    BE = aMaxBE;  // aMaxBE defined in Ieee802154Const.h
                // XXX edited by me
                if (BE > mpib.getMacMaxBE())
                    BE = mpib.getMacMaxBE();  // macMaxBE defined in MacPIB.c/.h
...
bool IEEE802154Mac::filter(mpdu* pdu)
{
    ...
    if (!mpib.getMacPromiscuousMode())
    {
        ...
        else
        {
            ...
            {
                if (!(pdu->getDest().isBroadcast()) && !(pdu->getDest().equals(myMacAddr)))
                {
...
void IEEE802154Mac::sendMCPSDataIndication(mpdu* rxData)
{
    ...
    delete(rxData); // XXX checken! ABER: mit decapsulate  oben wurde kopie von inhalt von rxData erstellt, object auf das rxData zeigt wird nicht mehr gebraucht
    return;
}
...
int IEEE802154Mac::calFrameByteLength(mpdu* frame)
{
    ...
    int MHRLength = calMacHeaderByteLength(((frame->getFcf() & damMask) >> damShift) + ((frame->getFcf() & samMask) >> samShift), (bool) ((frame->getFcf() && secuMask) >> secuShift));
...
void IEEE802154Mac::FSM_MCPS_DATA_request(phyState pStatus, MACenum mStatus)
{
    ...
    if (txOption == DIRECT_TRANS || txOption == 0)
    {
        switch (taskP.taskStep(task))
        {
            ...
            case 1: {
                if (pStatus == phy_IDLE)  // CSMA/CA succeeds
                {
                    ...
                    genSetTrxState(phy_TX_ON);
                      ...
                      // here simply retry
                {
                    macEV << " CSMA-CA reports CANNEL ACCESS FAILURE \n";
                    macEV << " retry ... \n";
                    csmacaResume();
                }
                break;
            ...
            case 3: {
                ...
                else  // time out when waiting for ACK
                {
                    ...
                    numDataRetry++;
                    // XXX edited by me: use value of MacPIB class
        ...
        switch (taskP.taskStep(task))
        {
            ...
            case 3: {
                ...
                else  // time out when waiting for ACK, normally impossible in GTS
                {
                    ...
                    numGTSRetry++;
                    //XXX edited by me: use value of MacPIB
