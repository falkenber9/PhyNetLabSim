...
simsignal_t IEEE802154Radio::numCollidedSignal = registerSignal("numCollided");
...
void IEEE802154Radio::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        IEEE802154RadioStateSignal = registerSignal("radioState");
        channelNumberSignal = registerSignal("channelNumber");
...
//        {
        ...
        // XXX ADDED BY ME
        cc->setRadioTXPower(myRadioRef, transmitterPower);
...
void IEEE802154Radio::sendUp(AirFrame *airframe)
{
    ...
    if (frame->getEncapsulatedPacket() != NULL)
    {
        ...
        delete(frame); // XXX nochmal ganz genau checken! ABER: mit decapsulate ganz oben wurde kopie von inhalt erstellt, frame wird nicht mehr gebraucht
    }
    else
        dataInd->encapsulate(frame);
    // XXX nochmal ganz genau checken! ABER: frame ist nun Teil von dataInd, NICHT löschen!

    send(dataInd, upperLayerOut);
    //delete(frame);  // removed due to issues
    delete(airframe); //  XXX nochmal ganz genau checken! ABER: mit decapsulate ganz oben wurde kopie von inhalt erstellt, airframe wird nicht mehr gebraucht
}
...
//            return;
            ...
            {
                radioEV << "------Receiver is already ON (IDLE)------------ \n";
            }
            ...
            {
                radioEV << "------Receiver turned ON (IDLE)------------ \n";
        ...
        case phy_CHANGE_TRANSMITTER_POWER: {
            ...
            if (newTransmitterPower != -1)
            {
                ...
                if (newTransmitterPower > getChannelControlPar("pMax").doubleValue())
                    ...
                    error("[RADIO]: got a Change Transmitter Power command with value greater then pMax of CC");
                else {
...
void IEEE802154Radio::handleLowerMsgStart(AirFrame* airframe)
{
    ...
    else
    {
        if(snrInfo.ptr != NULL)
        {
            radioEV << "Frame " << airframe->getName() << " caused a collision! \n";
            numCollided++;
        }
        radioEV << "Frame " << airframe->getName() << " is treated as noise! \n";
...
void IEEE802154Radio::handleLowerMsgEnd(AirFrame * airframe)
{
    ...
    if (snrInfo.ptr == airframe)
    {
        ...
        {
            cModule* host = this->getParentModule()->getParentModule();
            airframe->getEncapsulatedPacket()->getEncapsulatedPacket()->setKind(frameState); // fix for setting of PhyIndications
//            cPacket* tmp1 = airframe->decapsulate();
//            cPacket* tmp2 = tmp1->decapsulate();
//            tmp2->setKind(frameState);
//            tmp1->encapsulate(tmp2);
//            airframe->encapsulate(tmp1);
            airframe->setName(frameState == COLLISION ? "COLLISION" : "BITERROR"); // msg name not evaluated for now
            if (frameState == COLLISION)
            {
                host->bubble("Packets collided - frameState COLLISION");
                numCollided++; // packet destroyed by other is part of the collision too. packets caused the collision are counted in handleLowerMsgStart
                emit(numCollidedSignal, numCollided);
                numCollided = 0;
            }
            else if (frameState == BITERROR)
            {
                host->bubble("Packet got errors - frameState BITERROR");

            }
            numGivenUp++;
            //delete(airframe); //  XXX checken! ABER: mit decapsulate ganz oben wurde kopie von inhalt erstellt, airframe wird nicht mehr gebraucht
        }
        else
...
void IEEE802154Radio::performCCA()
{
    ...
    /// XXX EDITED by me
    simtime_t ccaDuration = (1/symbolRate)*8 ; // symbol period * aCcaDurartion
